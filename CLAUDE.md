...
The lifecycle per phase: `plan → execute → advance to next phase`. After all phases complete: `audit-milestone → plan-milestone-gaps (if gaps found) → execute gap phases → re-audit (loop until clean) → complete-milestone`. PROMPT.md explicitly forbids interactive GSD commands (`/gsd:discuss-phase`, `/gsd:verify-work`, `/gsd:progress`) because they require human input and would hang `--print` mode.
